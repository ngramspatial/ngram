...
export function setupResponseControl(button: HTMLButtonElement, send: () => void, stop: () => void) {
  let state = { agentState: 'idle', playing: false, compacting: false };
  ...
  button.addEventListener('click', () => { if (isActive()) stop(); else send(); });
  render();
  return {
    isActive,
    update(next: Partial<typeof state>) { state = { ...state, ...next }; render(); },
    reset() { state = { agentState: 'idle', playing: false, compacting: false }; render(); },
  };
}
